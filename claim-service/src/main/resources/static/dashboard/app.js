...
function renderClaims() {
  ...
  visible.forEach(function (claim) {
    ...
    var reason = cell(claim.claimType + " - " + claim.reason, "reason"); reason.title = claim.description;
    ...
    row.append(primary, cell(claim.policyNumber), reason, cell(currencyFormatter.format(claim.estimatedAmount), "money"), status, cell(dateFormatter.format(new Date(claim.submittedAt)), "date"));
    var actions = document.createElement("td"); var viewButton = document.createElement("button"); viewButton.type = "button"; viewButton.className = "icon-button table-icon"; viewButton.setAttribute("aria-label", "Open claim details"); viewButton.title = "Open claim details"; viewButton.innerHTML = '<i data-lucide="file-search"></i>'; viewButton.addEventListener("click", function () { openClaim(claim); }); actions.appendChild(viewButton); if (currentUser.role === "AGENT" && claim.status === "SUBMITTED") { ["Approve", "Reject"].forEach(function (decision) { var button = document.createElement("button"); button.type = "button"; button.className = "decision-button " + decision.toLowerCase(); button.textContent = decision; button.addEventListener("click", function () { openDecision(claim, decision === "Approve"); }); actions.appendChild(button); }); } row.appendChild(actions);
...
async function openClaim(claim) { document.getElementById("claim-detail-title").textContent = claim.claimType + " claim " + claim.id.slice(0, 8).toUpperCase(); document.getElementById("claim-detail-meta").textContent = claim.reason + " | " + currencyFormatter.format(claim.estimatedAmount) + "\n" + claim.description; document.getElementById("claim-timeline").replaceChildren(); document.getElementById("claim-dialog").showModal(); try { var events = await api("/claims/" + claim.id + "/timeline"); var timeline = document.getElementById("claim-timeline"); if (events.length === 0) { var empty = document.createElement("li"); empty.className = "timeline-empty"; empty.textContent = "This legacy claim has no recorded activity before the audit timeline was introduced."; timeline.appendChild(empty); return; } events.forEach(function (event) { var item = document.createElement("li"); var title = document.createElement("strong"); title.textContent = event.title; var detail = document.createElement("p"); detail.textContent = event.detail; var meta = document.createElement("span"); meta.textContent = event.actor + " | " + dateFormatter.format(new Date(event.occurredAt)); item.append(title, detail, meta); timeline.appendChild(item); }); } catch (error) { document.getElementById("claim-detail-meta").textContent = error.message; } }
...
document.getElementById("close-claim").addEventListener("click", function () { document.getElementById("claim-dialog").close(); });
...
window.addEventListener("load", async function () { lucide.createIcons(); try { showPortal(await api("/api/auth/me")); } catch (_) {} window.setInterval(function () { if (currentUser && !document.querySelector("dialog[open]") && !document.activeElement.matches("input, textarea, select")) refreshPortal(); }, 30000); });
