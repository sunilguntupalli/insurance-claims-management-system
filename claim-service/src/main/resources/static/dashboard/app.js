...
function renderClaims() {
  ...
  visible.forEach(function (claim) {
    ...
    row.addEventListener("click", function () { openClaim(claim); });
    ...
    if (currentUser.role === "AGENT") { var actions = document.createElement("td"); if (claim.status === "SUBMITTED") { ["Approve", "Reject"].forEach(function (decision) { var button = document.createElement("button"); button.type = "button"; button.className = "decision-button " + decision.toLowerCase(); button.textContent = decision; button.addEventListener("click", function (event) { event.stopPropagation(); openDecision(claim, decision === "Approve"); }); actions.appendChild(button); }); } row.appendChild(actions); }
...
async function openClaim(claim) { document.getElementById("claim-detail-title").textContent = claim.claimType + " claim " + claim.id.slice(0, 8).toUpperCase(); document.getElementById("claim-detail-meta").textContent = claim.reason + " | " + claim.description; document.getElementById("claim-timeline").replaceChildren(); document.getElementById("claim-dialog").showModal(); try { var events = await api("/claims/" + claim.id + "/timeline"); var timeline = document.getElementById("claim-timeline"); events.forEach(function (event) { var item = document.createElement("li"); var title = document.createElement("strong"); title.textContent = event.title; var detail = document.createElement("p"); detail.textContent = event.detail; var meta = document.createElement("span"); meta.textContent = event.actor + " | " + dateFormatter.format(new Date(event.occurredAt)); item.append(title, detail, meta); timeline.appendChild(item); }); } catch (error) { document.getElementById("claim-detail-meta").textContent = error.message; } }
